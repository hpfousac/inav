...
void mixTable(void)
{
    ...
    if (feature(FEATURE_3D)) {
        if (!ARMING_FLAG(ARMED)) throttlePrevious = 1000; // When disarmed set to mid_rc. It always results in positive direction after arming.

        if ((rcCommand[THROTTLE] <= (1000 - rcControlsConfig()->deadband3d_throttle))) { // Out of band handling
            throttleMax = flight3DConfig()->deadband3d_low;
            throttleMin = motorConfig()->minthrottle;
            throttlePrevious = throttleCommand = rcCommand[THROTTLE];
        } else if (rcCommand[THROTTLE] >= (1000 + rcControlsConfig()->deadband3d_throttle)) { // Positive handling
            throttleMax = motorConfig()->maxthrottle;
            throttleMin = flight3DConfig()->deadband3d_high;
            throttlePrevious = throttleCommand = rcCommand[THROTTLE];
        } else if ((throttlePrevious <= (1000 - rcControlsConfig()->deadband3d_throttle)))  { // Deadband handling from negative to positive
    ...
    if (ARMING_FLAG(ARMED)) {
        for (int i = 0; i < motorCount; i++) {
            ...
            if (failsafeIsActive()) {
                ...
                if (throttlePrevious <= (1500 - rcControlsConfig()->deadband3d_throttle)) {
            ...
            if (feature(FEATURE_MOTOR_STOP) && ARMING_FLAG(ARMED)) {
                ...
                bool userMotorStop = !navigationIsFlyingAutonomousMode() && !failsafeIsActive() && (rcData[THROTTLE] < 1000);
